...
class LoginAPI(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        instance = ActivityPeriod.objects.create(user=user, start_time=datetime.now(timezone(zone)))
        token = AuthToken.objects.create(user)[1]
...
class LogoutAPI(APIView):
    ...
    def get(self, request, format=None):
        ...
        zone = queryset.first().timezone
        queryset = ActivityPeriod.objects.filter(user=request.user.pk, end_time__isnull=True)
        queryset.update(end_time=datetime.now(timezone(zone)))
        logout(request)
